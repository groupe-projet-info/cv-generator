<template>
  <div id="container">
      ...
      <div>
        ...
        <ul class="no-style">
          <li class="sp-item">
            ...
            <span class="font-small">{{ cv.phoneNumber }}</span>
          </li><br>
          <li class="sp-item">
            <svg width="1.25em" height="1.25em" viewBox="0 0 16 16" class="bi" xmlns="http://www.w3.org/2000/svg">
              <path fill="white" d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2z" />
            ...
            <span class="font-small">{{ cv.emailAdress }}</span>
          </li><br>
          <li class="sp-item">
            <svg width="1.25em" height="1.25em" viewBox="0 0 16 16" class="bi" xmlns="http://www.w3.org/2000/svg">
              <path fill="white" d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2z" />
            ...
            <span class="font-small">{{ cv.homeAdress }}</span>
          </li><br>
          <li class="sp-item">
            <svg width="1.25em" height="1.25em" viewBox="0 0 16 16" class="bi" xmlns="http://www.w3.org/2000/svg">
              <path fill="white" d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2z" />
            ...
            <span class="font-small">{{ cv.drivingLicence }}</span>
          </li>
        </ul>
      </div>
      <div>
        <h3>Langues</h3>
        <p class="no-margin" v-for="lang in languages" :key="lang.id">{{ lang.languageName }} ({{ lang.level }})</p>
      </div>
      <div style="display: flex; flex-direction: column;">
        <h3>Compétences</h3>
        <ul class="no-style">
          <li class="sp-item" v-for="skill in skills" :key="skill.id">
            <span class="font-small">{{ skill.skillTitle }}: {{ skill.skillValue }}</span>
          </li>
        </ul>
      </div>
      <div style="display: flex; flex-direction: column;">
        <h3>Centres d'intérêt</h3>
        <div class="sp-item" v-for="hobby in cv.hobbies" :key="hobby">
    ...
    <div id="main-panel">
      ...
      <div class="main-item">
        <div class="item-header">
          <h2 class="no-margin">Certifications</h2>
        </div>
        <div class="item" v-for="certification in certifications" :key="certification.id">
          <div class="item-date">{{ certification.certificationPassYear }}</div>
          <div class="item-content no-margin no-padding c-lines">
            {{ certification.authority }}: {{ certification.certificationName }}
          </div>
        </div>
      </div>
    </div>
...
export default Vue.extend({
  ...
  async asyncData({ $api, route }) {
    try {
      ...
      let educations = await $api.education.getList(route.params.cv)
      let certifications = await $api.certification.getList(route.params.cv)
      let jobs = await $api.job.getList(route.params.cv)
      return { cv, languages, skills, educations, certifications, jobs }
    } catch (_err) {
      return {
        cv: {
          fullName: '',
